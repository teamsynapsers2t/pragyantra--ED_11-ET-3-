...
# Parse databaseUrl from api_key query param if present
...
def clean_postgres_url(url):
    if not url:
        return None
    # Strip query parameters except sslmode
...
# We will try both the original decoded url, and replacing 51214 with 51603
urls_to_try = [
...
try:
    cur = conn.cursor()
    # Query function source code from pg_proc
    cur.execute("""
    ...
    if res:
        ...
        print("\nfn_detect_root_flaws / fn_generate_weakness_report not found")
